<template>
  <div>
    <el-container style="position: absolute;top: 55px;bottom: 0;right: 0;left: 0;">
      <el-header class="d-flex align-items-center border-bottom">
        ...
        <el-button type="warning"
                   @click="unChoose"
                   size="medium"
                   v-if="chooseList.length > 0">取消选中</el-button>
        <el-button type="danger"
                   @click="imageDel({all:true})"
                   size="medium"
                   v-if="chooseList.length > 0">批量删除</el-button>
        <el-button type="success"
                   size="medium"
                   @click="opemAlbumModel(false)">创建相册</el-button>
        <el-button type="warning"
                   size="medium"
      ...
      <el-container>
        <el-aside width="200px"
          ...
          <ul class="list-group list-group-flush">
            <album-item v-for="(item,index) in albums"
                        :key="index"
                        :item="item"
                        :index="index"
                        :active="albumIndex == index"
                        @change="albumChange()"
                        @edit="opemAlbumModel({item,index})"
    ...
    <!-- 修改或者创建相册 -->
    <el-dialog :title="albumModelTitle"
    ...
    <el-dialog title="上传图片"
      ...
      <div class="text-center">
        <el-upload class="upload-demo w-100"
                   drag
